...
export const OfflineIndicator: React.FC = () => {
  ...
  if (justReconnected) {
    return (
      <div className="bg-emerald-600 text-white text-xs font-semibold px-3 py-1.5 text-center flex items-center justify-center gap-2 shadow-xs transition-all animate-in fade-in sticky top-0 z-50">
        ...
        <span>Ligação à internet restabelecida. Sincronizado com o servidor na nuvem!</span>
      </div>
    );
  }
  ...
  return (
    <div className="bg-red-600 text-white text-xs font-medium px-3 py-2 text-center flex items-center justify-between gap-2 shadow-md sticky top-0 z-50">
      <div className="flex items-center gap-2 mx-auto">
        <WifiOff className="w-4 h-4 shrink-0 animate-pulse" />
        <span>
          <strong>Sem Ligação à Internet:</strong> Ligue os dados móveis ou Wi-Fi para consultar preços em tempo real e sincronizar agendamentos diretamente na nuvem.
        </span>
      </div>
      <button
        type="button"
