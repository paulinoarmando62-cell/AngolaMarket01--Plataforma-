...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  // Customer contact state (never pre-filled with admin or staff credentials)
  const [fullName, setFullName] = useState((currentUser && currentUser.role === 'buyer') ? currentUser.name : '');
  const [phone, setPhone] = useState((currentUser && currentUser.role === 'buyer') ? currentUser.phone : '');
  const [alternatePhone, setAlternatePhone] = useState('');
  
  // Client email - strictly clean and empty by default (never auto-filled without user typing)
  ...
  const [neighborhood, setNeighborhood] = useState((currentUser && currentUser.role === 'buyer' && currentUser.defaultNeighborhood) || selectedZone.neighborhood || '');
  const [streetAddress, setStreetAddress] = useState((currentUser && currentUser.role === 'buyer' && currentUser.defaultStreetAddress) || '');
  const [referencePoint, setReferencePoint] = useState((currentUser && currentUser.role === 'buyer' && currentUser.defaultReferencePoint) || '');
  ...
  // Affiliate code: ONLY populated if explicitly passed from an active URL referral link!
  const [affiliateCode, setAffiliateCode] = useState((affiliateRefCode && affiliateRefCode.trim()) ? affiliateRefCode.trim().toUpperCase() : '');
  const [hasError, setHasError] = useState<string | null>(null);

  // Sync if affiliateRefCode is provided via URL
  useEffect(() => {
    if (isOpen) {
      if (affiliateRefCode && affiliateRefCode.trim()) {
  ...
  // Sync if an authenticated buyer opens checkout
  useEffect(() => {
    if (isOpen && currentUser && currentUser.role === 'buyer') {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newCustomerAccount = (!currentUser && fullName.trim() && phone.trim()) ? {
      name: fullName.trim(),
      phone: phone.trim(),
      password: '123456',
      email: clientEmail.trim() || undefined,
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between pb-1 border-b border-stone-100">
              <div className="flex items-center gap-2 text-sm font-bold text-stone-900">
                ...
                <span>1. Dados do Destinatário da Entrega em Luanda</span>
              </div>
              {currentUser && currentUser.role === 'buyer' ? (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (currentUser.defaultNeighborhood) setNeighborhood(currentUser.defaultNeighborhood);
                    if (currentUser.defaultStreetAddress) setStreetAddress(currentUser.defaultStreetAddress);
                    if (currentUser.defaultReferencePoint) setReferencePoint(currentUser.defaultReferencePoint);
                  }}
                  className="text-[11px] font-bold text-red-600 hover:text-red-700 bg-red-50 hover:bg-red-100 px-2.5 py-1 rounded-xl transition-all cursor-pointer"
                >
                  Usar meus dados salvos ({currentUser.name})
                </button>
              ) : (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="space-y-1 sm:col-span-2">
                <label className="text-xs text-stone-700 font-bold flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-stone-400 font-normal">Para envio do comprovativo / fatura digital</span>
                </label>
                <input
                  type="email"
                  name="checkout_clean_customer_email"
                  autoComplete="off"
                  value={clientEmail}
