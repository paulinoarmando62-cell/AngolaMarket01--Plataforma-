...
export const SellerPortalModal: React.FC<SellerPortalModalProps> = ({
  ...
  const [sellerName, setSellerName] = useState('Minha Loja Luanda');
  const [sellerLocation, setSellerLocation] = useState('São Paulo & Maianga, Luanda');
  const [sellerPhone, setSellerPhone] = useState('+244 923 000 000');
  const [stockCount, setStockCount] = useState('10');
  const [description, setDescription] = useState('');
  const [featuresText, setFeaturesText] = useState('Garantia de 30 dias\nEntrega rápida em Luanda\nPagamento TPA no ato da entrega');
